package com.kostyarazboynik.todoapp.utils.notifications
...
class NotificationsReceiver : BroadcastReceiver() {
    ...
    @Inject
    lateinit var scheduler: NotificationsScheduler
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            ...
            coroutineScope.launch(Dispatchers.IO) {
                val toDoItem = repository.getToDoItemById(id = id)
                scheduler.cancel(id = toDoItem.id)
                createManager(context).notify(
                    toDoItem.id.hashCode(),
                    buildNotification(context = context, toDoItem = toDoItem)
                )
            }
        } catch (_: Exception) {
        }
