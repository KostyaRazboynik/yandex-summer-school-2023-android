...
class SwipeCallback(
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        if (direction == ItemTouchHelper.LEFT) setDeleteCallback(viewHolder = viewHolder)
        else if (direction == ItemTouchHelper.RIGHT) setChangeDoneCallback(viewHolder = viewHolder)
    }
    ...
    private fun setDeleteCallback(viewHolder: RecyclerView.ViewHolder) =
        (viewHolder as? ToDoItemViewHolder)?.todoItem?.let { swipeCallback.onDelete(toDoItem = it) }

    private fun setChangeDoneCallback(viewHolder: RecyclerView.ViewHolder) =
        (viewHolder as? ToDoItemViewHolder)?.todoItem?.let { swipeCallback.onChangeDone(toDoItem = it) }
}